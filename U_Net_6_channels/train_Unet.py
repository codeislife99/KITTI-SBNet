...
# user_name = 'codeislife99'
user_name = 'teamd'
mask_root_train = '/media/'+str(user_name)+'/New Volume/VLR_Project/U_Net_6_channels/gt_masks_raw/train'
img_root_train = '/media/'+str(user_name)+ '/New Volume/VLR_Project/U_Net_6_channels/bev_raw/train'
mask_root_val = '/media/'+str(user_name)+'/New Volume/VLR_Project/U_Net_6_channels/gt_masks_raw/val'
img_root_val = '/media/'+str(user_name)+'/New Volume/VLR_Project/U_Net_6_channels/bev_raw/val'
...
def make_dataset(mode):
    ...
    for filename in sorted(os.listdir(mask_root)):
        ...
        img_file1 = os.path.join(img_root, filename[:-4]+"_0.jpg")
        img_file2 = os.path.join(img_root, filename[:-4]+"_1.jpg")
        items.append((img_file1,img_file2, mask_file))
...
class ImageDataset(Dataset): #Defining the class to load datasets
    ...
    def __getitem__(self,idx):
        ...
        img_path1,img_path2, mask_path = self.imgs[idx]
    ...
    if epoch%10 == 0:
        for j,datapoint_1 in enumerate(validation_loader): #for validation
            ...
            if torch.cuda.is_available():
                input_image_1 = Variable(datapoint_1['image'].cuda(),volatile = True)
                output_image_1 = Variable(datapoint_1['masks'].cuda(),volatile = True)

            else:
                input_image_1 = Variable(datapoint_1['image'],volatile = True)
                output_image_1 = Variable(datapoint_1['masks'],volatile = True)
